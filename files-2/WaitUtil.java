 ...
 *       Works for buttons, inputs, checkboxes/radios, and anchors.</li>
...
public final class WaitUtil {
    ...
    private static final Duration POLL_INTERVAL = WAIT_TIME.dividedBy(30); // ~1s
     ...
     * <ul>
     *   <li>All elements: waits for {@code isEnabled()}. Deliberately does
     *       NOT require {@code isDisplayed()} - custom checkbox/radio
     *       styling (including GOV.UK Design System) commonly keeps the
     *       real {@code <input>} visually hidden while a styled label
     *       represents it, and that input is still fully clickable.</li>
    ...
    /**
     * Tag-aware readiness check backing {@link #waitAndClick}. Applies
     * {@code isEnabled()} for every tag, plus an anchor-specific check for
     * common "visually disabled" signals that {@code isEnabled()} can't see.
     *
